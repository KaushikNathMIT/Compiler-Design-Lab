...
int statement() {
        ...
        if(assignStat()) {
                ...
                //else tpos = skipToSynchronizingToken();
                fseek(fp, -strlen(token), SEEK_CUR);
                errorFound=1;
                printf("\nMissing semicolon");
                return 1;
        ...
        if (decisionStat()) return 1;
        fseek(fp,tpos,SEEK_SET);
        if (loopingStat()) return 1;
        fseek(fp,tpos,SEEK_SET);
        return 0;
}
...
int skipToSynchronizingToken() {
        errorFound=1;
        printf("\nSkipping to synchronizing token");
        while(strcmp(getNextWord(fp),";"))
        return ftell(fp);
}
...
int main() {
        ...
        if(errorFound==1) printf("\nError found and skipped.");
        return 0;
}
